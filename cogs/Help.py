...
class Help(commands.Cog):
    ...
    async def help(self, ctx, message:str = None):
        if message:
            embed = discord.Embed(title = f"Command Info: {message}", description = self.command_dict[message].short_doc, colour = discord.Color.red() )         
            ...
            embed.add_field(name = "Other", value= "```define, lottery, profile```", inline= False)
